...
namespace Local.Integration.Scripts.Game
{
    public class LegRaycast : MonoBehaviour
    {
        ...
        private bool _isIKTargetInitialized;

        void Start()
        {
            _IKPos = IKTarget.position;
            _isIKTargetInitialized = true;
        }
        ...
        void Update()
        {
            ...
            if (_isIKTargetInitialized && Vector3.Distance(IKTarget.position, _currentHitPos) > MaxDistance)
            {
